...
import type { DBField, DBRecord, DBTable } from '@/lib/types';
...
export default function EditRecordModal({ table, record, allTables, onClose, onSave }: Props) {
  const editableFields = table.fields.filter(f => !f.isSystem || f.name !== 'id');

  const [values, setValues] = useState<Record<string, any>>(() => {
    const init: Record<string, any> = {};
    editableFields.forEach(f => {
      init[f.id] = record.values[f.id] ?? (f.type === 'boolean' ? false : '');
    });
    return init;
  });
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-base font-semibold text-gray-900">レコードを編集</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
          ...
          <div className="text-xs text-gray-400 bg-gray-50 rounded-lg px-3 py-2 space-y-0.5">
            <p>ID: <span className="font-mono text-gray-500">{record.id}</span></p>
            <p>作成: {new Date(record.createdAt).toLocaleString('ja-JP')}</p>
            <p>更新: {new Date(record.updatedAt).toLocaleString('ja-JP')}</p>
          </div>

          {editableFields.length === 0 && (
            <p className="text-sm text-gray-400 text-center py-4">編集可能な項目がありません</p>
          )}
